...
def main():
    ...
    with Session(engine) as session:
        ...
        session.commit()
        print("✅ Character system: Alignment, Race, SubRace, Class_, Background, Skills, Stat, Actor")
        ...
        session.commit()
        print("✅ Character relationships: ActorAOnBRelations, ActorToSkills, ActorToRace, ActorToClass, ActorToStat")
        ...
        session.add(schema.FactionAOnBRelations(faction_a_id=1, faction_b_id=2, setting_id=1))
        ...
        session.add(schema.LocationCityDistricts(location_id=1, district_id=3, setting_id=1))
        session.add(schema.Resident(actor_id=1, location_id=1, setting_id=1))
        session.add(schema.LocationFloraFauna(location_id=1, name="Test Plant", setting_id=1))
        session.commit()
        print("✅ Location system: Location, LocationToFaction, LocationDungeon, LocationCity, LocationCityDistricts, Resident, LocationFloraFauna")
        ...
        session.commit()
        print("✅ History system: History, HistoryActor, HistoryLocation, HistoryFaction, HistoryObject")

        # World data system tables
        world_data1 = schema.WorldData(id=1, name="Test Lore", setting_id=1)
        session.add(world_data1)
        
        session.add(schema.HistoryWorldData(history_id=1, world_data_id=1, setting_id=1))
        session.commit()
        ...
        plot_section1 = schema.LitographyPlotSection(id=1, plot_section_type=schema.PlotSectionType.RISING, plot_id=1)
        session.add(plot_section1)
        
        node1 = schema.LitographyNode(id=1, node_type=schema.NodeType.EXPOSITION, x_position=0.0, y_position=0.0, storyline_id=1)
        session.add(node1)
        
        node2 = schema.LitographyNode(id=2, node_type=schema.NodeType.ACTION, x_position=100.0, y_position=0.0, storyline_id=1)
        session.add(node2)
        
        session.add(NodeConnection(output_node_id=1, input_node_id=2))
        session.add(schema.LitographyNodeToPlotSection(node_id=1, litography_plot_section_id=1))
        
        note1 = schema.LitographyNotes(id=1, title="Test Note", note_type=schema.NoteType.OTHER, linked_node=node1, storyline_id=1)
        session.add(note1)
        
        session.commit()
        print("✅ Story plotting: LitographyPlot, LitographyPlotSection, LitographyNode, NodeConnection, LitographyNodeToPlotSection, LitographyNotes")

        # Character arc system tables
        arc_type1 = schema.ArcType(id=1, name="Test Arc Type", setting_id=1)
        session.add(arc_type1)
        
        arc1 = schema.LitographyArc(id=1, title="Test Arc", arc_type_id=1, storyline_id=1)
        session.add(arc1)
        
        arc_point1 = schema.ArcPoint(id=1, arc_id=1, node_id=1, order_index=1, title="Test Point")
        session.add(arc_point1)
        
        session.add(schema.ArcToNode(arc_id=1, node_id=1))
        session.add(schema.ArcToActor(actor_id=1, arc_id=1))
        session.commit()
        print("✅ Character arc system: ArcType, LitographyArc, ArcPoint, ArcToNode, ArcToActor")
