...
def check_duplicate_entry(session: Session, table_class, filtered_data: dict, target_setting_id: int) -> bool:
...
def import_table_data(session: Session, table_name: str, table_data: list, target_setting_id: int, id_mapping: dict):
    ...
    for row_data in table_data:
        # Skip empty rows
        if not any(v for v in row_data.values() if v not in ["", 0, 1, 0.0, 1.0, False, True, None]):
            continue
        ...
        try:
            ...
            if check_duplicate_entry(session, table_class, filtered_data, target_setting_id):
                print(f"⚠️  Skipping duplicate {table_name}: {filtered_data.get('name', 'unnamed')}")
                skipped_count += 1
...
def import_world_building_package(json_file_path: str, target_setting_id: int) -> bool:
    ...
    with Session(engine) as session:
        ...
        table_order = [
            ...
            'actor_a_on_b_relations', 'actor_to_skills', 'actor_to_race', 'actor_to_class', 'actor_to_stat',
            'faction_a_on_b_relations', 'faction_members',
            'location_to_faction', 'location_dungeon', 'location_city', 'location_city_districts',
            'residents', 'location_flora_fauna', 'location_a_on_b_relations',
            'location_geographic_relations', 'location_political_relations', 'location_economic_relations',
            'location_hierarchy', 'history_actor', 'history_location', 'history_faction',
            'history_object', 'object_to_owner', 'history_world_data'
        ]
        ...
        for table_name in table_order:
            if table_name in package_data:
                ...
                if isinstance(table_data, list) and table_data:
                    imported_count = import_table_data(session, table_name, table_data, target_setting_id, id_mapping)
                    if imported_count > 0:
        ...
        for table_name, table_data in package_data.items():
            if table_name.startswith('_'):  # Skip metadata
                continue
            if table_name not in table_order and isinstance(table_data, list) and table_data:
                imported_count = import_table_data(session, table_name, table_data, target_setting_id, id_mapping)
                if imported_count > 0:
                    print(f"✅ Imported {imported_count} rows into {table_name}")
                    total_imported += imported_count
        ...
        try:
            session.commit()
            print(f"🎉 Successfully imported {total_imported} total rows from {package_name}!")
            return True
        except Exception as e:
...
def main():
    ...
    import sys
    if len(sys.argv) != 3:
        print("Usage: python import_world_building.py <json_file_path> <target_setting_id>")
