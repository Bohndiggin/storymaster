...
class MainWindowController:
    ...
    def validate_ui_database_sync(self):
        ...
        try:
            ...
            db_nodes = self.model.get_litography_nodes(project_id=self.current_project_id)
    ...
    def setup_node_editing_panel(self):
        ...
        if current_layout:
            ...
            self.node_panel.setStyleSheet("background-color: #1e1e1e; border-left: 1px solid #424242;")
            ...
            self.delete_node_btn.setStyleSheet("QPushButton { background-color: #8B0000; }")
    ...
    def setup_plot_section_selector(self, parent_layout):
        ...
        section_container.setStyleSheet("background-color: #1a1a1a; border-bottom: 1px solid #424242;")
    ...
    def delete_section(self, tab_index):
        ...
        if tab_index < len(self.section_tab_ids):
            ...
            if nodes_in_section:
                self.view.ui.statusbar.showMessage(f"Cannot delete section with {len(nodes_in_section)} nodes", 5000)
                return
        ...
        try:
            ...
            with Session(self.model.engine) as session:
                ...
                sections = session.query(LitographyPlotSection).filter_by(
                    plot_id=self.current_plot_id
                ).order_by(LitographyPlotSection.id).all()
                ...
                current_section = session.query(LitographyNodeToPlotSection).filter_by(
    ...
    def on_save_node_changes(self):
        ...
        try:
            ...
            new_section = self.section_combo_sections[section_index].id if section_index >= 0 and hasattr(self, 'section_combo_sections') and section_index < len(self.section_combo_sections) else None
    ...
    def on_delete_node(self):
        ...
        try:
            ...
            with Session(self.model.engine) as session:
                ...
                notes_to_delete = session.query(LitographyNotes).filter_by(
                ...
                node_to_delete = session.query(LitographyNode).filter_by(
                    id=self.selected_node.id,
                    project_id=self.current_project_id
                ).first()
                ...
                if node_to_delete:
                    ...
                    session.commit()
                    self.view.ui.statusbar.showMessage("Node and associated notes deleted successfully", 3000)
                else:
        ...
        try:
            ...
            if position_type == "start":
                ...
                if self.current_plot_section_id:
                    all_nodes_after = self.model.get_litography_nodes(project_id=self.current_project_id)
            ...
            if position_type == "before" and reference_node_id:
                ...
                if not reference_node:
                    self.view.ui.statusbar.showMessage("Reference node no longer exists", 3000)
                    self.load_and_draw_nodes()
                    return
            ...
            all_nodes_after = self.model.get_litography_nodes(project_id=self.current_project_id)
            new_node = max(all_nodes_after, key=lambda n: n.id)  # Get the node with highest ID (newest)
            ...
            self.view.ui.statusbar.showMessage(f"Added new node {position_type} existing node", 3000)
            self.load_and_draw_nodes()
    ...
    def on_delete_node_button_clicked(self, node_id):
        ...
        try:
            ...
            all_db_nodes = self.model.get_litography_nodes(project_id=self.current_project_id)
            existing_ids = [n.id for n in all_db_nodes]
            
            # First verify the node exists in the database directly
            
            with Session(self.model.engine) as session:
                node_to_delete_db = session.query(LitographyNode).filter_by(
                    id=node_id,
                    project_id=self.current_project_id
                ).first()
                
                if not node_to_delete_db:
                    self.view.ui.statusbar.showMessage(f"Node {node_id} no longer exists, refreshing...", 3000)
            ...
            all_current_nodes = self.model.get_litography_nodes(project_id=self.current_project_id)
            existing_node_ids = {n.id for n in all_current_nodes}
            
            
            # Only update connections if the referenced nodes actually exist
            if (node_to_delete.previous_node and node_to_delete.previous_node in existing_node_ids and
                node_to_delete.next_node and node_to_delete.next_node in existing_node_ids):
            ...
            elif node_to_delete.previous_node and node_to_delete.previous_node in existing_node_ids:
            ...
            elif node_to_delete.next_node and node_to_delete.next_node in existing_node_ids:
    ...
    def add_node_to_section(self, node_id, section_id):
        ...
        try:
            ...
            with Session(self.model.engine) as session:
                ...
                existing = session.query(LitographyNodeToPlotSection).filter_by(
                    node_id=node_id,
                    litography_plot_section_id=section_id
                ).first()
                ...
                verification = session.query(LitographyNodeToPlotSection).filter_by(
                    node_id=node_id,
                    litography_plot_section_id=section_id
                ).first()
                ...
                if verification:
                    ...
                    print(f"Failed to verify link between node {node_id} and section {section_id}")
        except Exception as e:
    ...
    def move_node_to_section(self, node_id, new_section_id):
        ...
        try:
            ...
            with Session(self.model.engine) as session:
                ...
                existing_links = session.query(LitographyNodeToPlotSection).filter_by(
                    node_id=node_id
                ).all()
                ...
                new_link = LitographyNodeToPlotSection(
                    node_id=node_id,
                    litography_plot_section_id=new_section_id
                )
    ...
    def load_and_draw_nodes(self):
        ...
        for i, node in enumerate(ordered_nodes):
            ...
            node_item = create_node_item(x_pos, y_pos, 80, 80, node, self)  # Changed to 80x80 for square
        ...
        for node in ordered_nodes:
            if node.next_node and node.next_node in node_positions:
                ...
                if dx != 0 or dy != 0:
                    ...
                    # Draw arrow lines
                    arrow_line1 = QGraphicsLineItem(end_pos[0], end_pos[1], arrow_p1_x, arrow_p1_y)
                    arrow_line2 = QGraphicsLineItem(end_pos[0], end_pos[1], arrow_p2_x, arrow_p2_y)
