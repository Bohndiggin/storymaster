...
from PyQt6.QtWidgets import (
    ...
    QListWidget, QListWidgetItem, QMessageBox, QGroupBox, QComboBox
)
...
class SettingStorylinesDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.current_setting_label.setStyleSheet("font-weight: bold; font-size: 14px; color: #af80f8;")
        ...
        self.available_list.setToolTip("Storylines that can be connected to this setting")
        ...
        self.disconnect_btn.setToolTip("Disconnect selected storyline from this setting")
        ...
        self.connected_list.itemSelectionChanged.connect(self.on_connected_selection_changed)
    ...
    def on_setting_changed(self):
        ...
        if setting_id is None:
            self.current_setting_id = None
            self.current_setting_name = ""
            self.current_setting_label.setText("Select a setting to manage its storylines")
            self.set_content_enabled(False)
            return
        
        self.current_setting_id = setting_id
        self.current_setting_name = self.setting_combo.currentText()
        self.current_setting_label.setText(f"Managing storylines for: {self.current_setting_name}")
    ...
    def load_storylines(self):
        ...
        try:
            ...
            available_storylines = self.model.get_available_storylines_for_setting(self.current_setting_id)
            ...
            connected_storylines = self.model.get_storylines_for_setting(self.current_setting_id)
    ...
    def connect_storyline(self):
        ...
        try:
            success = self.model.link_storyline_to_setting(storyline_id, self.current_setting_id)
            if success:
    ...
    def disconnect_storyline(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                success = self.model.unlink_storyline_from_setting(storyline_id, self.current_setting_id)
                if success:
                    self.load_storylines()  # Refresh both lists
                    QMessageBox.information(
                        self, 
                ...
                QMessageBox.critical(self, "Error", f"Failed to disconnect storyline: {e}")