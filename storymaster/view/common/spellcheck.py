...
from PyQt6.QtWidgets import (
    QTextEdit, QLineEdit, QMenu, QApplication
)
...
class SpellChecker:
    ...
    def _load_custom_dictionary(self):
        ...
        creative_words = {
            ...
            "worldbuilding", "backstory", "storyline", "subplot", "protagonists",
            "antagonist", "deuteragonist", "antiheroes", "archetype", "trope",
            "foreshadowing", "MacGuffin", "deus", "machina", "denouement",
            
            # Character terms
            "characterization", "backstories", "archetypes", "personas",
            "motivations", "flaws", "quirks", "mannerisms",
            
            # Plot terms
            "inciting", "midpoint", "climax", "resolution", "subplot",
            "flashback", "flashforward", "montage", "voiceover",
            
            # Genre terms
            "fantasy", "scifi", "dystopian", "steampunk", "cyberpunk",
            "paranormal", "supernatural", "magical", "mystical",
            
            # Common proper nouns that authors use
            "Storymaster", "Lorekeeper", "Litographer"
        }
        ...
        try:
            custom_dict_path = os.path.expanduser("~/.local/share/storymaster/custom_dictionary.txt")
...
class BasicWordList:
    ...
    def __init__(self):
        ...
        self.words = set([
            ...
            "character", "story", "plot", "scene", "chapter", "book",
            "write", "writing", "author", "novel", "fiction", "fantasy",
            "dialogue", "description", "narrative", "protagonist", "villain"
        ])
...
class SpellCheckLineEdit(QLineEdit):
    ...
    def _update_spelling_style(self):
        ...
        if self.misspelled_words:
            self.setToolTip(f"Possible misspellings: {', '.join(self.misspelled_words)}")
        else:
            self.setToolTip("")
                ...
                if suggestions:
                    for suggestion in suggestions[:5]:  # Show top 5 suggestions
                        ...
                        action.triggered.connect(
                            lambda checked, s=suggestion, w=word_at_cursor: self._replace_word(w, s)
                        )
...
class SpellCheckTextEdit(QTextEdit):
    ...
    def contextMenuEvent(self, e):
        ...
        if word and not self.spell_checker.is_word_correct(word):
            ...
            add_action.triggered.connect(
                lambda: self._add_word_to_dictionary(word)
            )
            menu.addAction(add_action)
        ...
        menu.addSeparator()
        toggle_action = QAction("Enable Spell Check" if not self.spell_checker.enabled else "Disable Spell Check", self)
            ...
            if word and not spell_checker.is_word_correct(word):
                ...
                if suggestions:
                    for suggestion in suggestions[:5]:
                        ...
                        action.triggered.connect(
                            lambda checked, s=suggestion: _replace_word_in_widget(widget, s)
                        )
                        menu.addAction(action)
                    menu.addSeparator()
