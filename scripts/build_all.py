...
def main():
    ...
    if platform.system() == "Darwin":
        build_targets.append(
            ("build_macos.py --non-interactive", "macOS App Bundle & DMG")
        )
    else:
        # Cross-compilation option for macOS
        build_targets.append(
            ("build_macos.py --non-interactive", "macOS App Bundle (Cross-compile)")
        )
            ...
            try:
                ...
                if choice == str(len(build_targets) + 1):
                    ...
                    if 1 <= choice_num <= len(build_targets):
                        ...
                        print(
                            f"Please enter a number between 1 and {len(build_targets) + 1}"
                        )
            except ValueError:
